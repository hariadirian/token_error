...
class ApiSetupOrgManagementController extends Controller
{
    ...
    public function getDataTable(Request $request){
        ...
        // MAIN QUERY
        $datas = M_Us_Backend_Organizations::where('state', 'Y');
        ...
        $datas->where('created_at', '<=', $date_to);
        $datas->where('created_at', '>=', $date_from);

        // FILTER WHERE
        if($request->search['value'] != ''){
            $search = strtoupper($request->search['value']);
            $datas->where(function($q) use($datas, $search){
                $q->orWhere('organization_name', 'LIKE', '%'.$search.'%');
                $q->orWhere('organization_type', 'LIKE', '%'.$search.'%');
        ...
        if($request->order[0]['column'] == 0){
            $column = 'id_us_backend_organization';
        ...
        $datas->select(DB::raw('@row:=@row+1 AS no'), 'cd_us_backend_organization', 'organization_name','description','organization_type', 'created_at', 'created_by',  'is_active', 'm_attributeset_uu');
        ...
        $all_attributeset = MV_Product::whereNotNull('attributeset_uu')->get()->keyBy('attributeset_uu');
        if($datas){
            foreach($datas as $key => $data){

                $rows[$x][0] = $data->cd_us_backend_organization;
                $rows[$x][1] = $data->no;
                $rows[$x][2] = $data->organization_name;
                $rows[$x][3] = $data->organization_type;
                $rows[$x][4] = $data->m_attributeset_uu? isset($all_attributeset[$data->m_attributeset_uu])? $all_attributeset[$data->m_attributeset_uu]->attributeset_name : '<span style="color:#a7cdef"><i>Tidak Ditemukan</i></span>' : '<span style="color:#a7cdef"><i>Belum Diisi</i></span>';
    ...
    public function createOrgModal(){
        
        $attributeset = MV_Product::whereNotNull('attributeset_uu')->pluck('name', 'attributeset_uu');;

        $datas = [
            'attributeset'         => $attributeset,
        ];

        $modal_content = preg_replace( "/\r|\n/", "", view('backend.setup_org_management.modal-create-org', $datas) );
