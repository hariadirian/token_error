...
class ApiTicketController extends Controller
{
    ...
    public function getDataTable(Request $request){
        ...
        $datas = M_Ordered_Ticket_Txes::leftJoin('tmii_et_generated_ticket_hd as d', 
                ...
                ->where('tmii_et_ordered_ticket_txes.state', 'Y')
                ->where('paid_state', '2');
// print_r($datas->get()->first()->toCartProductHd->first()->summaryCartHd()->get());die;
        ...
        $datas->select(DB::raw('@row:=@row+1 AS no'), 'tmii_et_ordered_ticket_txes.id_et_ordered_ticket_txes','tmii_et_ordered_ticket_txes.id_et_cart_product_hd','tmii_et_ordered_ticket_txes.cd_et_ordered_ticket_txes', 'tmii_et_ordered_ticket_txes.created_at', 'd.created_at as generate_created_at', 'paid_at', 'e.email');
       ...
       if($datas){
            foreach($datas as $key => $data){
                if ($data->toCartProductHd) {
                    ...
                    $rows[$x][3] = $data->email;
                    $rows[$x][4] = $product;
                    $rows[$x][5] = isset($summaryCartHd->total_amount)? 'Rp. ' . number_format($summaryCartHd->total_amount, 0, ',', '.').',-' : '';
                    $rows[$x][6] =  $data->generate_created_at;
    ...
    public function getDetailDataTable(Request $request){
        ...
        $modal_content = preg_replace( "/\r|\n/", "", view('backend.ticket.datatable-detail-ticket', $data) );
    ...
    public function scanningTicket(Request $request){
        ...
        if(!$ticket_cart){
            $product = MV_Product::where('attributeset_uu', $ticket->toCartProductDt->m_attributeset_uu)->first();
            return json_encode(['state' => 0, 'message' => 'QRcode ini berlaku di wahana '.$product->attributeset_name.' Tunjukkan QRcode lainnya!' /*'Ticket is found! But you have no authorization to scanning this ticket. Check the ticket and make sure you are in the right vehicle!'*/]);
        }
